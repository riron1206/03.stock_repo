...
if __name__ == '__main__':
    ...
    if start_date is None:
        start_date = datetime.datetime.strptime(end_date, '%Y-%m-%d').date() - datetime.timedelta(days=600)  # 600日前からスタート
        start_date = start_date.strftime("%Y-%m-%d")

    # 指定銘柄についてシュミレーション
    df_profit_codes = None
    for code in tqdm(codes):
        df_profit, _ = trade(code, args['start_date'], args['end_date'],
